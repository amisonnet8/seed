...
## Seed言語仕様とAMIVM-IRの対応関係で検討が必要な論点

`seed_spec.md`の意味論をAMIVM-IRへどう落とし込むかは自明ではない箇所がある。実装時に設計判断が必要。

- **`null`とベース値**: Seedの変数は宣言直後`null`を取りうり、読み取るとベース値(`0`/`""`/`false`等)として振る舞う。Goの型のゼロ値とベース値は偶然一致するが、`isnull()`判定のためには「代入済みかどうか」を別途追跡する必要がある(例: 各変数ごとに付随するbool、またはSeed側の型を`(値, 代入済みフラグ)`のペアとして扱う設計等)。`File`型の`null`(どのファイルにも紐づいていない状態)も同様に検討が必要
- **配列の要素数不一致規則**(4節): 配列リテラルの代入・関数戻り値の代入いずれでも「超過切り捨て・不足は`null`埋め」という規則がある。AMIVM-IRの配列は固定長のみのため、この切り詰め/埋め処理はSeedのコード生成時にループ(`ASET`/`AGET`+`GOTO`)として展開する必要がある
- **配列の参照渡し**(7節): Seedでは関数引数の配列は常に参照渡し。AMIVM-IRでは`ADDR`/`PGET`/`PSET`やスライス(`SLTYPE`)を使った表現が候補になる
- **`main`の特別扱い**: Seedの`main`はエントリーポイント専用で他関数から呼び出せない制約がある一方、AMIVM-IR側の`!main`は`FUNC`の一種として扱われる。呼び出し禁止はSeed側の意味検査(スコープ解決)で保証すること
- **`+`演算子の型分岐**(5節): `Int+Int`→`ADD`、`Float+Float`→`ADD`、`String+String`→`CONCAT`と、Seedの`+`は演算対象の型によって生成するIR命令自体が変わる。異なる型同士の`+`はSeedの型チェック段階で弾く(amivm側は関与しない)
- **`read()`のEOF表現**: ファイル終端で`null`を返す仕様。Goの`bufio.Scanner`等は`(string, bool)`や`io.EOF`エラーで表現するため、Seedランタイム側の`read`実装で「EOFならSeedの`null`相当を返す」変換が必要

これらはSeed実装が進むにつれて確定していく設計判断であり、確定した内容は本ファイルまたは実装コード側のコメントとして残すこと(仮説段階のまま放置しない)。
