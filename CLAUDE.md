...
- チャネル・スライス・map・構造体・クロージャーは、対応する`TYPE`系命令(`SLTYPE`/`MPTYPE`/`STTYPE`/`FNTYPE`/`CHTYPE`)で型を定義してから使う(この5つは必須)。固定長配列だけは例外で、`^[n]type1`という複合形でインラインに宣言でき、名前付き宣言用の`ARTYPE`(4.16節)は使わなくてもよい選択肢に過ぎない
